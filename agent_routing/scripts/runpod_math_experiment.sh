#!/usr/bin/env bash
# Run one predeclared arm/seed; external tests require MARGENT_EVALUATE_TESTS=1.
# Start the frozen advisor separately; set CUDA_VISIBLE_DEVICES for the manager.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."
MARGENT_ARM="${1:-dynamic_sft}"
MARGENT_SEED="${2:-42}"
MARGENT_RUN_ROOT="${MARGENT_RUN_ROOT:-/workspace/margent-runs}"
MARGENT_DATA="${MARGENT_DATA:-/workspace/margent-data}"
case "$MARGENT_ARM" in dynamic_sft|success_sft|static_sft) ;; *) printf 'Unknown arm: %s\n' "$MARGENT_ARM" >&2; exit 2 ;; esac
MARGENT_BASE_CONFIG="${MARGENT_BASE_CONFIG:-$MARGENT_RUN_ROOT/frozen_math.json}"
MARGENT_ROUNDS="${MARGENT_ROUNDS:-2}"
MARGENT_CONFIG="$MARGENT_RUN_ROOT/configs/${MARGENT_ARM}_s${MARGENT_SEED}.json"
python - "$MARGENT_CONFIG" "$MARGENT_SEED" "$MARGENT_BASE_CONFIG" <<'PY'
import json, sys
...
python -m src.verifiable loop --config "$MARGENT_CONFIG" --data-dir "$MARGENT_DATA" \
  --out "$MARGENT_OUTPUT" --arm "$MARGENT_ARM" --rounds "$MARGENT_ROUNDS" --resume
if [[ "${MARGENT_EVALUATE_TESTS:-0}" == "1" ]]; then
